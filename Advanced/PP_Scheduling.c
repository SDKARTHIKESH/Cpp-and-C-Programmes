...
void priorityPreemptive(Process processes[], int len1, Output result[], int *len2);
...
int main() {
    ...
    priorityPreemptive(processes, len1, result, &len2);
...
void priorityPreemptive(Process processes[], int len1, Output result[], int *len2){
    ...
    while(completed != len1){
        ...
        for(int x = 0; x < len1; x++){
            if (processes[x].at <= time && isCompleted[x] == 0 && processes[x].priority < min){
        ...
        rt[shortest]--;
        min = processes[shortest].priority;
